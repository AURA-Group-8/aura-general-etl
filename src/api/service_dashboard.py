from src.databases import read_from_warehouse_db 
import pandas as pd
...
def get_agendamentos_semanais():
    ...
    resultado = read_from_warehouse_db(query)
...
def get_resumo():
    ...
    porcentagem_crescimento_tkm = calcular_porcentagem_crescimento(tkm_e_faturamento_por_mes["tkm"], tkm_e_faturamento_por_mes_passado["tkm"])
    porcentagem_crescimento_receita = calcular_porcentagem_crescimento(tkm_e_faturamento_por_mes["total_faturado"], tkm_e_faturamento_por_mes_passado["total_faturado"])
    porcentagem_crescimento_custos = calcular_porcentagem_crescimento(custos_por_mes["total_custos"], custos_por_mes_passado["total_custos"])
...
def get_tkm_e_faturamento_por_mes(mes_referencia: pd.Timestamp, data_limite: pd.Timestamp = None):
    ...
    return {
        "total_faturado": row.total_faturado or 0,
        "tkm": row.tkm or 0
    }
...
def get_custos_por_mes(mes_referencia: pd.Timestamp, data_limite: pd.Timestamp = None):
    ...
    return {
        "total_custos": row.total_custos or 0
    }
...
def get_faturamento(periodo_meses: int = None):
    ...
    lista = [
        {
            "mes": row.mes.strftime('%Y-%m-01') if hasattr(row.mes, 'strftime') else str(row.mes),
...
def get_top_servicos(periodo_meses: int = None):
    ...
    resultado_total = read_from_warehouse_db(query_total)
    total_execucoes = resultado_total[0].total_execucoes if resultado_total and resultado_total[0].total_execucoes else 0
    ...
    lista = [
        {
            ...
            "porcentagem_total": round((float(row.qtd_realizados or 0) / total_execucoes) * 100, 2)
        }
        for row in resultado
    ]
    ...
    lista = [
        {
            ...
            "ultimo_agendamento": row.ultimo_agendamento.strftime('%Y-%m-%d') if hasattr(row.ultimo_agendamento, 'strftime') else str(row.ultimo_agendamento),
